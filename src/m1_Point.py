...
class Point(object):
    ...
    def get_number_of_moves_made(self, dx, dy):
        count = 0
        if dx:
            count = count + 1
        if dy:
    ...
    def run_test_get_distance_from_start(self, x, y):
        d = math.sqrt((self.x - x) + (self.y - y))
        return d
    ...
    def closer_to(self, p2, p3):
        ...
        d_p2 = math.sqrt((self.x - p2.x) + (self.y - p2.y))
        d_p3 = math.sqrt((self.x - p3.x) + (self.y - p3.y))
        if d_p2 <= d_p3:
            return p2
        else:
            return p3
    ...
    def halfway_to(self, p2):
        point_x = (self.x + p2.x) / 2
        point_y = (self.y + p2.y) / 2
        print('Point' "is" '(',  point_x, ',', point_y, ")")
